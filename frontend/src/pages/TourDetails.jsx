  ...
  useEffect(() => {
    ...
    const fetchTourAndBookings = async () => {
      try {
        ...
        if (token) {
          try {
            const bookingsResponse = await axios.get(`http://localhost:5001/api/bookings/tour/${id}`, {
              headers: { Authorization: `Bearer ${token}` }
            });
            setUserBookings(bookingsResponse.data);
          } catch (bookingError) {
            console.log('Could not fetch bookings (may require authentication):', bookingError);
          }
        }
